...
def render_card(result: dict) -> str:
    ...
    # 뱃지
    badge = (
        '<div class="badge-ok">✓ 논문 근거 있음</div>'
        if has_evidence
        else '<div class="badge-none">✗ 직접 근거 없음</div>'
    )
    ...
    return (
        '<div class="res-card">'
        "<h3>🧬 BioRAG 분석 리포트</h3>"
        f"{badge}{body}{sources}{term_html}{meta}"
        "</div>"
    )


# ── 질문 처리 함수 (예시 버튼 + 직접 입력 공용) ──

def process_question(question: str):
    """질문을 백엔드에 보내고, 메시지 기록에 저장하고, 화면에 렌더링한다."""
    # 유저 메시지 추가
